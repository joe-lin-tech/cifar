...
def train_resnet(epochs: int = 50, batch_size: int = 128, learning_rate: float = 0.1,
    ...
    for epoch in range(epochs):
        ...
        if ckpt_frequency > 0 and (epoch + 1) % ckpt_frequency == 0:
            ...
            torch.save({
                ...
                'val_loss': val_loss,
            }, os.path.join(save_folder, f"cifar_resnet{f'_fold{fold + 1}' if fold > -1 else ''}_epoch{epoch + 1}.pt"))
    ...
    torch.save({
        ...
        'test_loss': test_loss,
        'test_acc': test_acc
    }, os.path.join(save_folder, f"cifar_resnet{f'_fold{fold + 1}' if fold > -1 else ''}.pt"))